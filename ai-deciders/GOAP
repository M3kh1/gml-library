...
function plannerGOAP() constructor
{
	...
	checkKeysMatch = function(_source, _target)
	{
	    var _keys = struct_get_names(_source);
	    for (var i = 0; i < array_length(_keys); i++)
	    {
	        var _key = _keys[i];
	        if (!keyMatches(_source, _target, _key))
	        {
	            return false;
	        }
	    }
	...
	keyMatches = function(_source, _target, key)
	{
	    return struct_exists(_source, key) && _source[$ key] == _target[$ key];
	}

	simulateReactions = function(state, effects)
	{
